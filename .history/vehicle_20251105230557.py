...
class Vehicle:
    ...
    def update(self,traci):
        ...
        if(self.state == VehicleState.SendingRequest):#iterate over all cars in target lane and send nearest ones a request
            for veh in traci.lane.getLastStepVehicleIDs("E1_" + str(self.targetLane)):
                checkLanePos = traci.vehicle.getLanePosition(veh)
                if(checkLanePos < (self.lanePos + self.length) and checkLanePos > (self.lanePos - self.length)):
